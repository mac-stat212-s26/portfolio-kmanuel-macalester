...
mn_cities <- st_read("/Users/katiemanuel/Desktop/spring 26/stat212/portfolio-kmanuel-macalester/ica/04_adv_maps/data/shp_loc_pop_centers/city_and_township_population_centers.shp")
mn_water <- st_read("/Users/katiemanuel/Desktop/spring 26/stat212/portfolio-kmanuel-macalester/ica/04_adv_maps/data/shp_water_lakes_rivers/LakesAndRivers.shp")
```
...
transform_and_plot_ph <- function(spatial_obj, new_crs) {
    spatial_obj |> 
        st_transform(crs = new_crs) |> 
        ggplot() +
...
transform_and_plot_aus <- function(spatial_obj, new_crs) {
    spatial_obj |> 
        st_transform(crs = new_crs) |> 
        ggplot() +
...
transform_and_plot_ph(nc, new_crs = "EPSG:3121")
transform_and_plot_aus(nc, new_crs = "EPSG:3112")
```
...
If I had point location data not in the NAD27 CRS, I would expect the accuracy to be extremely low when overlaid on the original North Carolina map. The further your CRS is from your location of interest, the less accurate it will be. 
...
```{r}
st_crs(mn_cities) #NAD83, EPSG = 26915
st_crs(mn_counties) #WGS 84, EPSG = 4326
st_crs(mn_water) #NAD83, EPSG = 26915
...
An advantage of using this function for making color palettes is that it is color-blind friendly (for common forms of color-blindness), and the color maps are uniform in black-and-white. The difference between the variants is that they are used for different types of data: _d() is for discrete data, _c() is for continuous data, and _b() is for binning continuous data before mapping. 
...
png("/Users/katiemanuel/Desktop/spring 26/stat212/portfolio-kmanuel-macalester/ica/04_adv_maps/figures/tc_map_zoom.png", width = 800, height = 400)
ggplot() +
    geom_raster(data = elev_df, aes(x = x, y = y, fill = elevation)) + 
    geom_sf(data = mn_counties, fill = NA, color = "black") + # county boundary layer
    geom_sf(data = mn_water, fill = "lightsteelblue1", color = "lightsteelblue1") + # NEW: river/lake layer
