...
#3
# Prediction: If I use ==, I will get an error that it cannot be equal to more than one thing.
# Wrong Way: 
diamonds |> 
  mutate(is_cut = cut == c("Fair", "Premium", "Ideal")) |> 
  summarize(sum_cut = sum(is_cut), mean_cut = mean(is_cut))

# Correct Way:
diamonds |> 
  mutate(is_cut = cut %in% c("Fair", "Premium", "Ideal")) |> 
  summarize(sum_cut = sum(is_cut), mean_cut = mean(is_cut))
