...
```{r}
phonedigits <- function(numbers) {
  numbers <- str_remove_all(numbers, "[^[:alnum:]]")
  area <- str_sub(numbers, 1, 3)
  num3 <- str_sub(numbers, 4, 6)
  num4 <- str_sub(numbers, 7, 10)
  all <- str_c("(",area,")", num3, "-", num4 )
}

phonedigits(c("651-330-8661", "6516966000", "800 867 5309"))
```
...
The error above is because you need to call making before you can call data. The positional matching is not correct. 
...
```{r}
temp_conversion <- function(temp, unit) {
  if(unit == "F") {
    (temp - 32)*5/9}
  else if(unit == "C") {
    (temp * 9/5) + 32}
...
temp_conversion(0, unit = "C")
temp_conversion(32, unit = "F")
```
...
scatter(diamonds, x = carat, y = price)
```
## Done!
